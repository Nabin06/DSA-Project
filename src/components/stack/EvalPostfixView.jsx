...
import { evalPostfix } from '../../lib/stackAlgorithms.js'

export default function EvalPostfixView() {
  const [expr, setExpr] = useState('3 4 + 2 * 7 1 - /')
  const [run, setRun] = useState(null)
  const [idx, setIdx] = useState(0)

  const handleRun = () => {
    const result = evalPostfix(expr)
    setRun(result)
    setIdx(0)
  }
  ...
  return (
    <div>
      ...
      <div className="mono mt-2 text-xxs text-muted">
        Space-separated tokens: numbers and operators. Example: 3 4 + 2 * 7 1 - /
      </div>

      <div className="mt-5 grid grid-cols-1 gap-5 md:grid-cols-[220px_1fr]">
        <div className="space-y-4">
          <div>
            <div className="mono mb-2 text-xxs text-muted">POSTFIX TOKENS</div>
          ...
          <StackBox title="OPERAND STACK" items={current?.stack ?? []} color="stack" />
        </div>
